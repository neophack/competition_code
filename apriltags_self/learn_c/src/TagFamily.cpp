...
cv::Mat_<TagFamily::byte> TagFamily::makeImage(size_t id) const {
 ...
 // cout<<rval<<endl;
  ...
  //bool flag =v&(code_t(1)<<code_t(bits-1));
  return rval;

}

TagFamily::code_t TagFamily::makecode(cv::Mat_<byte> rval)
{
    code_t v;
    int bb = whiteBorder + blackBorder;
    const byte white = 1;
    ...
    for (uint y = 0; y < d; y++) {
      for (uint x = 0; x < d; x++) {
          if(rval(y+bb,x+bb) = black)
        (v&(code_t(1)<<code_t(bits-1)))!=0;
         v = v<<code_t(1);
      }
    }
    return v;
...
cv::Mat TagFamily::detectionImage(const TagDetection& det,
  ...
  if (im.channels() < dst.channels()) {
    cv::Mat i2;
    //cv::cvtColor(im, i2,CV_BGR2GRAY);
     cvtColor(im, i2, CV_BGR2GRAY);  
    im = i2;
  }
